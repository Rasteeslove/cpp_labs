...
private:
	...
	double * GetCorners()
	{
		...
		for (int i = 0; i < 8; i += 2)
			corners[i] = centerX + lengthSign[i / 2] * (length / 2) * cos(PI * angle / 180) + widthSign[i / 2] * (width / 2) * sin(PI * angle / 180);

		for (int i = 1; i < 8; i += 2)
			corners[i] = centerY + lengthSign[i / 2] * (length / 2) * sin(PI * angle / 180) - widthSign[i / 2] * (width / 2) * cos(PI * angle / 180);
